...
# 将所有附属的日志和配置也全部塞进系统的隐藏临时目录，实现真正的 0 落盘！
BASE_DIR="/tmp/.system_kworker_cache"
LOG_DIR="${BASE_DIR}"
RUN_DIR="${BASE_DIR}"

APP_LOG="${LOG_DIR}/kworker.log"
APP_PID_FILE="${RUN_DIR}/kworker.pid"
...
uninstall_app() {
  ...
  if [ -z "${NON_INTERACTIVE:-}" ]; then
    printf "%b" "${YELLOW}此操作将杀掉幽灵进程并清理内存缓存，确定继续吗? [y/N]: ${NC}"
  ...
  say "正在停止运行中的隐蔽程序..."
  stop_if_running "${APP_PID_FILE}" "幽灵进程"
  
  say "正在清理内存中的隐藏记录目录: ${BASE_DIR}"
  rm -rf "${BASE_DIR}"
  rm -f "${APP_BIN}"
...
show_menu() {
  printf "%b\n" "${GREEN}========================================${NC}"
  printf "%b\n" "${GREEN} OneImg (绝对洁癖无痕版) 一键管理脚本 ${NC}"
  printf "%b\n" "${GREEN}========================================${NC}"
